...
export function Header() {
  const [currentUrl, setCurrentUrl] = useState("http://localhost:3000/news");
  const [currentIcon, setCurrentIcon] = useState(newsIcon);

  useEffect(() => {
    const path = window.location.pathname;
    if (path === "/news") {
      setCurrentUrl("http://localhost:3000/");
      setCurrentIcon(chatIcon);
    } else {
      setCurrentUrl("http://localhost:3000/news");
      setCurrentIcon(newsIcon);
    }
  }, []);
    ...
    <>
      <header className="sticky top-0 z-50 flex items-center justify-between w-full px-4 border-b h-14 shrink-0 dark:bg-slate-800 bg-white backdrop-blur-xl">
        <span className="inline-flex items-center home-links whitespace-nowrap">
            <span className="block sm:inline text-lg sm:text-xl lg:text-2xl font-semibold dark:text-white text-black">MED <span className="linear-wipe">TECH</span></span>
        </span>
        <div className="flex items-center justify-end space-x-2">
          <Button variant="outline" asChild>
            ...
            >
              <img src={currentIcon.src} alt="Icon" className="w-6 h-6" />
            </a>
          </Button>
        </div>
      </header>
