...
export default function Page() {
  ...
  const handleFollowUpClick = useCallback(async (question: string) => {
    ...
    await handleUserMessageSubmission(question, "market_trends");
  }, []);
  ...
  const handleSubmit = async (message: string, category: string) => {
    if (!message) return;
    await handleUserMessageSubmission(message, category);
  };
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    setInputValue('');
    await handleSubmit(messageToSend, "market_trends");
  };

  const handleUserMessageSubmission = async (userMessage: string, category: string): Promise<void> => {
    ...
    try {
      const streamableValue = await myAction(userMessage, category);
        ...
        setMessages((prevMessages) => {
          ...
          if (messageIndex !== -1) {
            ...
            if (typedMessage.followUp) {
              currentMessage.followUp = { followUp: typedMessage.followUp.followUp };
            }
    ...
    <div>
      ...
      <div className={`px-2 fixed inset-x-0 bottom-0 w-full bg-gradient-to-b duration-300 ease-in-out animate-in dark:from-gray-900/10 dark:from-10% peer-[[data-state=open]]:group-[]:lg:pl-[250px] peer-[[data-state=open]]:group-[]:xl:pl-[300px]] mb-4 bring-to-front`}>
        <div className="mx-auto max-w-xl sm:px-4 ">
          {messages.length === 0 && (
            <InitialQueries questions={['L\'intelligence artificielle au Maroc', 'Actualités sur la monétique au Maroc', 'Y a-t-il de nouvelles startups lancées au Maroc ?']} handleFollowUpClick={handleFollowUpClick} />
          )}
          <form
            ref={formRef}
