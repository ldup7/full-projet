...
import { serperSearch } from './tools/searchProviders'; // Import serperSearch function
...
// Function to get content from the top 10 blue links
...
// Function to process and vectorize content
...
async function myAction(userMessage: string, category: string): Promise<any> {
  ...
  (async () => {
      ...
      const searchResults = sources.map(source => `${source.title} (${source.link})`).join(", ");

      const systemMessageResponse = await axios.post('http://localhost:5000/process-system-message', { 
          query: userMessage, 
      ...
      const chatCompletion = await openai.chat.completions.create({
          messages: [
              { role: "system", content: systemMessageResponse.data.system_message },
